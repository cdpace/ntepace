    ...
    this.processView = function (viewpath, params, callback) {
        try {
            loadView(viewpath, function (viewContent) {
                ...
                //Process Arguments
                var argumentsPattern = /@{1}[\w]+/igm;
                var viewVariables = output.match(argumentsPattern);

                if (viewVariables != null) {
                    viewVariables.forEach(function (variable) {

                        var variableCleanName = variable.replace("@", "");

                        if (params[variableCleanName] != null) {
                            output = output.replace(variable, params[variableCleanName]);
                        } else {
